# Module 7: Multi-Platform Gateway
...
| | |
|---|---|
| **What it is** | A single daemon process that connects Hermes to Telegram, Discord, Slack, WhatsApp, Signal, and 15+ other platforms — all from one agent instance |
...
| **Setup** | `hermes gateway setup` — interactive wizard, or manual `.env` config |
| **Service** | `hermes gateway install` — installs as systemd (Linux) or launchd (macOS) |
...
Without the gateway, you run Hermes in your terminal — useful, but tied to your desktop. With the gateway:
...
SMS      ─┤   adapters   │  (single daemon)    │──→ Tools (shell, files, web)
Matrix   ─┤              │  Sessions, cron,    │──→ Skills
Home     ─┤              │   security, voice   │──→ Memory
Assistant─┘              └─────────────────────┘
```
...
Each platform has its own **adapter** — a Python module that handles the platform-specific protocol (Telegram's polling/webhook API, Discord's WebSocket gateway, Signal's CLI bridge, etc.).

When a message arrives:

1. **Authorization check** — Is this user allowed? (Allowlist, DM pairing, or admin check)
2. **Session lookup** — Does this chat have an existing conversation context?
3. **Platform-specific handling** — Mention checks, free-response channels, thread isolation
4. **Agent dispatch** — The message is sent to the AIAgent with the correct session context
5. **Response delivery** — The agent's response is streamed back to the platform with typing indicators

### Same Tools Everywhere

Every platform gets the same toolset — just with a different toolset name for logging:
...
- **Daily** — Reset at a specific hour (default: 4:00 AM)
- **Idle** — Reset after N minutes of inactivity (default: 1440 min)
- **Both** — Whichever triggers first (default)
- **None** — Never auto-reset (controlled by compression only)
...
| Command | Purpose |
|---------|---------|
| `/setdescription` | What the bot can do — shown before a user chats |
...
3. Note your **Application ID** — you'll need it for the invite URL
...
Still on the **Bot** page, click **Reset Token**. Copy it immediately — it's only shown once.
...
Open the invite URL → select your server → **Authorize**. The bot appears in your server's member list.
...
/platforms — Show all active platform adapters
```
...
- **Admin** — full access to all slash commands and capabilities
- **Regular user** — can chat normally but only run explicitly enabled slash commands
...
The easiest way — in any chat, use:
...
**Common causes:**
- **Wrong bot token** — Verify in `.env`
- **User not authorized** — Is your user ID in `TELEGRAM_ALLOWED_USERS`?
- **Privacy mode blocking** — If the bot is in a group, check privacy mode
- **Rate limiting** — Telegram rate-limits bots that send too many requests. Wait a few minutes.
...
The breaker does **not** auto-resume — this prevents the gateway from thrashing reconnects during a sustained outage.
...
1. `~/.hermes/logs/gateway.log` — search for `circuit breaker`, `paused`, or `disabled`
2. `/platform list` output — shows current state and last reason
3. The platform's status page (Telegram API status, Discord status, etc.)

Once upstream is healthy, use `/platform resume <name>` to re-arm the adapter.
...
- Each platform has its own **adapter** — but all share the same agent, tools, memory, and skills
- **Telegram** setup: BotFather → get token → `hermes gateway setup` → done
- **Discord** setup: Developer Portal → create bot → **enable Message Content Intent** → invite → configure
