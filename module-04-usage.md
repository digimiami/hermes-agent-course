# Module 4: Daily Usage & Power Features
...
| `--continue <name>` | Resume a named session |
...
| `/background <prompt>` | `/bg`, `/btw` | Run a prompt in a background thread |
| `/queue <prompt>` | `/q` | Queue input for next turn |
...
| Toggle YOLO | `/yolo` |
| Rollback file changes | `/rollback <N>` |
| Fork conversation | `/branch "exploration-name"` |
| See all sessions | `hermes sessions list` |
| Send webhook | `curl -X POST <gateway-url>/webhook -d '{"message":"..."}'` |
